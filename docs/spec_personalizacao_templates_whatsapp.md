...
A solução garante retrocompatibilidade e resiliência total através de **fallback automático** para textos padrão em caso de campos não preenchidos, **validação visual e bloqueio suave da tag `{link}`** para assegurar o funcionamento do autoatendimento no Canal do Cliente, um **editor em Split View com Live Preview no formato de balão nativo do WhatsApp**, e um mecanismo de **disparo de teste instantâneo no número real do gerente**.
...
5. Como **Gerente da barbearia**, quero receber um alerta visual destacado caso eu remova a tag `{link}` do texto, para evitar enviar mensagens sem a URL de autoatendimento.
6. Como **Gerente da barbearia**, quero que o botão de salvar fique desabilitado enquanto a tag `{link}` estiver ausente no editor, para garantir que meus clientes sempre consigam acessar o Canal do Cliente para gerenciar horários.
...
- **Obrigatoriedade de Link:**
  - A interface valida a presença da expressão regular `/\{link\}/i` no texto antes de habilitar a gravação.
...
- **Módulos Testados:**
  1. `src/modules/whatsapp/templates.ts`: Testes unitários puros cobrindo interpolação de todas as variáveis, substituição de dados simulados, validação estrita da tag `{link}` e integridade das constantes de fallback.
  2. `src/pages/gerente/Whatsapp.tsx`: Testes de componentes React (Vitest + Testing Library) validando:
     ...
     - Exibição de alerta e desabilitação do botão de salvar quando `{link}` for removido.
