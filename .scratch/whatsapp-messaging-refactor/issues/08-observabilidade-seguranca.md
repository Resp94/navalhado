...
- [x] O adapter possui timeout explícito e classifica erros retryable e permanentes de forma consistente.
- [x] Respostas externas não expõem corpo bruto ou credenciais da UAZAPI.
- [ ] Falhas de RPC, renderização, outbox, worker e provider podem ser diferenciadas operacionalmente.
- [x] Funções privilegiadas mantêm search path explícito e grants mínimos.
- [x] A autorização impede envio manual ou processamento de evento de outro tenant.
