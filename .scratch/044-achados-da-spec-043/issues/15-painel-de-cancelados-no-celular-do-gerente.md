...
- [ ] Verificado no navegador em largura de celular — não verificado, ver nota abaixo
...
- **Não verificado no navegador**: esta sessão não tem uma aba já autenticada como gerente (diferente do que ocorreu no ticket 11, numa sessão anterior); abrir uma nova exige login, e a regra permanente deste projeto proíbe eu digitar senha em qualquer campo, mesmo de teste. Tentei abrir o preview desta sessão (`preview_start` com `navalhado-dev`) para pelo menos conferir a tela de login/layout responsivo, mas a navegação para a porta dinâmica do servidor foi recusada pela ferramenta de navegador nesta sessão (`navigation ... denied or failed`), então nem a tela pública carregou. A cobertura de tela (63 testes, incluindo o describe novo) é a prova disponível; browser fica pendente para quando houver uma sessão autenticada disponível.
