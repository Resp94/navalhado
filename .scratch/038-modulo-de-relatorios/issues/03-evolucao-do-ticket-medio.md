...
- [x] O contrato de Faturamento passa a devolver ticket médio nos totais do período, nos totais do
      período anterior e em cada agrupamento.
- [x] O contrato devolve a lista de ticket por profissional: profissional, se está ativo ou
      arquivado, líquido, Comandas distintas e ticket médio, incluindo inativos e arquivados com item
      no período.
- [x] Agrupamento ou período sem Comanda fechada devolve ticket vazio.
- [x] Comanda com dois profissionais conta uma Comanda para cada um, com o valor de cada item para
      quem executou.
- [x] Adaptador preserva o vazio (não converte em zero).
- [x] Página Faturamento ganha cartão de ticket médio com variação vs período anterior, coluna de
...
- [x] Casos novos no pgTAP `33_relatorio_faturamento` (plano ajustado), teste do adaptador e caso
