...
- [x] Função privada de Receita Reconhecida de Item recebe tenant e intervalo e devolve, por item de
...
- [x] Contrato de leitura do Faturamento recebe tenant, data inicial, data final e granularidade, e
...
- [x] Padrões de banco: retorno em `jsonb`; funções pública e núcleo declarados `STABLE`; função
...
- [x] Cada fonte agregada na própria CTE antes de juntar com as outras (faturamento e, mais adiante,
      recebido no mesmo agrupamento): juntar duas fontes com várias linhas por agrupamento no mesmo
      `group by` multiplica os totais.
- [x] Recorte de período como intervalo meio aberto sobre a coluna, sem envolver coluna indexada em
...
- [x] Acesso: barbeiro recusado; gerente pedindo outro tenant recusado; gerente com tenant nulo
      recusado; `proprietario` com o tratamento das RPCs financeiras existentes.
- [x] Validação com mensagens em pt-BR e código de parâmetro inválido: datas nulas, fim antes do
...
- [x] Agrupamento: semana começa na segunda-feira, mês civil, primeiro e último agrupamento recortados
      ao período, cada agrupamento com as próprias datas.
- [x] Período anterior = os N dias imediatamente anteriores ao início, com N igual à extensão do
      período, podendo passar do limite de 730 dias.
- [x] Comanda aberta e cancelada não contam; item revertido não conta; item indisponível conta pelo
      preço total; gorjeta fica fora do líquido e aparece em gorjetas.
- [x] Comanda fechada às 23h30 no horário local cai no dia local, e não no dia UTC.
- [x] Teste cruzado: para o mesmo intervalo, o líquido é igual à receita operacional e a qualidade do
      dado é igual à qualidade histórica de `get_tenant_financial_metrics`.
- [x] Índice de Comandas por tenant e fechamento, parcial nas fechadas, criado com "se não existir" e
      sem `concurrently` (não funciona dentro da transação da migração); nenhuma função existente
      alterada.
- [x] Arquivo pgTAP `33_relatorio_faturamento` cobrindo acesso, validação, fuso, reconhecimento,
...
- [x] `explain (analyze, buffers)` do contrato num período de 366 dias registrado nas notas do
      ticket, confirmando uso do índice novo e folga em relação ao limite de 8 segundos do papel
      autenticado.
- [x] Módulo de relatórios com interface do adaptador, repositório que valida período e granularidade
...
- [x] Funções puras: atalhos de período (este mês como padrão, mês passado, últimos 30 dias, últimos
...
- [x] Item "Relatórios" no menu lateral, logo depois de Financeiro. Em largura de celular (até
      768px, o limite que o painel já usa) o item não aparece na barra inferior nem na gaveta "Mais".
- [x] Módulo exclusivo do desktop: em largura de celular, qualquer rota `/relatorios/*` mostra só o
...
- [x] Rotas `/relatorios` (catálogo) e `/relatorios/faturamento`; sub-rota desconhecida volta ao
      catálogo; rotas atrás do Gatekeeper de Onboarding; barbeiro não alcança o módulo.
- [x] Catálogo sem números, com os dez relatórios agrupados por página e uma frase de pergunta em
      cada um; os que ainda não existem aparecem sem link ou marcados como em breve.
- [x] Layout do módulo com título, navegação entre páginas e filtro de período (atalho, datas,
      granularidade) preservado na URL ao trocar de página.
- [x] Página Faturamento com cartões de resumo (bruto, descontos, líquido, serviços, produtos,
      ...
      erro.
- [x] Nenhum layout de celular (sem lista de cartões); em desktop estreito a tabela rola na horizontal
      dentro do próprio contêiner, nunca a página.
- [x] Recarrega ao mudar filtro, ao voltar o foco para a janela e pelo botão "Atualizar", sem
      realtime.
- [x] `CONTEXT.md` ganha Módulo de Relatórios (exclusivo do desktop) e Receita Reconhecida de Item;
      ...
      privadas.
- [x] Testes de módulo: repositório (validação e mensagens), atalhos incluindo virada de mês, virada
      ...
      item "Relatórios".
- [x] `npm run test` verde e pgTAP verde via MCP do Supabase (begin + migration + teste + rollback).
