...
- [x] Função privada de Visita em `language sql`, `STABLE`, retornando conjunto e sem concessão para
...
- [x] Contrato de leitura recebe tenant, faixa de atraso opcional, profissional opcional, limite
...
- [x] Mesmo padrão de função pública, núcleo privado com relógio injetado e acesso do ticket 01;
      validação de limite, deslocamento e faixa com mensagens em pt-BR.
- [x] Prazo = menor Tempo de Retorno entre os serviços da última Visita; sem serviço ou sem prazo, 20
      dias, como o lembrete de retorno existente.
- [x] Cliente sem Retorno = dias desde a última Visita maior que o prazo e nenhum Agendamento
      pendente ou confirmado depois de agora.
- [x] Faixas de atraso: até 15, 16 a 30, 31 a 60, mais de 60 dias.
- [x] Totais e faixas ignoram paginação e filtro de faixa, mas respeitam o filtro de profissional.
- [x] Índice de Agendamentos por tenant, cliente e início criado se ausente (hoje só existe índice por
      cliente isolado), com "se não existir" e sem `concurrently`.
- [x] Retorno em `jsonb`, com totais e faixas fora da lista: eles precisam chegar mesmo quando a
...
- [x] `explain (analyze, buffers)` da consulta com a base do dev registrado nas notas do ticket,
      confirmando uso do índice novo.
- [x] Arquivo pgTAP `37_relatorio_clientes_sem_retorno` cobrindo acesso, prazo pelo menor serviço e
...
- [x] Repositório, adaptador e hook da página, com testes de validação de paginação e conversão.
- [x] Rota `/relatorios/clientes-sem-retorno`, que esconde o filtro de período do layout e mostra os
      próprios filtros; catálogo passa a linkar o relatório.
- [x] Página com cartões de totais e faixas, tabela paginada, ação de abrir a Central 360º, ação de
...
- [x] Teste da página com repositório falso cobrindo paginação e WhatsApp indisponível sem telefone.
- [x] `CONTEXT.md` ganha Visita e Cliente sem Retorno.
