...
export function SegmentedControl<T extends string | number = string>({
  ...
  return (
    <div
      role="tablist"
      aria-label={ariaLabel}
      className={`inline-flex items-center bg-bg-primary shadow-[0_0_0_0.5px_var(--color-text-primary)] rounded-md p-[3px] gap-[3px] box-border select-none overflow-x-auto ${fullWidth ? 'flex w-full' : ''} ${size === 'sm' ? 'min-h-9' : 'min-h-[42px]'} ${className}`}
      style={style}
    >
      {options.map((option) => {
