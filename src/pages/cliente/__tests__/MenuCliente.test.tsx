...
describe('MenuCliente - TDD', () => {
  ...
  it('exibe botões de Remarcar e Cancelar para agendamentos ativos na aba Próximos horários', async () => {
    const mockDetails = {
  ...
  it('exibe modal de redirecionamento para o WhatsApp caso o prazo de cancelamento tenha expirado', async () => {
    ...
    const confirmCancelBtn = await screen.findByRole('button', { name: /Sim, confirmar cancelamento/i });
    fireEvent.click(confirmCancelBtn);

    const expiredHeading = await screen.findByText(/Prazo de alteração expirado/i);
    expect(expiredHeading).toBeInTheDocument();

    const waButton = screen.getByRole('link', { name: /Falar no WhatsApp/i });
    expect(waButton).toBeInTheDocument();
    expect(waButton).toHaveAttribute('href', expect.stringContaining('5592999999999'));
  });

  it('resolve token válido, lista agendamentos e navega para novo agendamento', async () => {
    const mockDetails = {
    ...
    expect(screen.getByText(/Olá, Cliente Validado/)).toBeInTheDocument();
  ...
  it('exibe sessão pública e permite iniciar novo agendamento', async () => {
    localStorage.clear();
    const publicDetails = {
    ...
    const publicAppointment = {
      appointment_id: 'app-public-1',
      start_time: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
      end_time: new Date(Date.now() + 210 * 60 * 1000).toISOString(),
      status: 'confirmed',
      payment_status: 'pending',
      cancellation_reason: null,
      professional_name: 'Barbeiro Público',
      professional_id: 'prof-public',
      service_name: 'Corte Público',
      service_id: 'serv-public',
      service_price: 50,
      service_duration: 30,
      tenant_name: 'Barbearia Pública',
      tenant_id: 'tenant-public',
