...
function respostaBase(overrides: Partial<RelatorioFaturamento> = {}): RelatorioFaturamento {
  return {
    ...
    totals: { gross: 500, discounts: 20, net: 480, services_net: 400, products_net: 80, tips: 30, closed_comandas: 2 },
    previous_totals: { gross: 300, discounts: 10, net: 290, services_net: 250, products_net: 40, tips: 15, closed_comandas: 2 },
    buckets: [
      {
...
describe('FaturamentoPage', () => {
  ...
  it('mostra estado vazio quando não há Comanda fechada no período', async () => {
    const adapter = new FakeRelatoriosAdapter(async () =>
      respostaBase({
        totals: { gross: 0, discounts: 0, net: 0, services_net: 0, products_net: 0, tips: 0, closed_comandas: 0 },
        buckets: [],
      })
    );
    const repository = new RelatoriosRepository(adapter);
