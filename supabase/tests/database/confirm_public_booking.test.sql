...
select ok((select exists(select 1 from pg_proc p where p.oid='public.confirm_public_booking(text,uuid,uuid,date,text,text,text,uuid)'::regprocedure and array_to_string(p.proconfig, ',') like '%search_path=%')),'booking confirmation pins its search path');
...
select ok((select exists(select 1 from pg_indexes where schemaname='public' and tablename='customers' and indexdef like '%telefone_normalizado%' and indexdef like '%UNIQUE%')),'customer identity has a unique tenant phone constraint');
...
values
  ('68000000-0000-0000-0000-000000000001','Confirmation Test','confirmation-test@test.local','92999990028','confirmation-test','{"monday":{"active":true,"start":"09:00","end":"17:00"}}'::jsonb,'America/Manaus',30,0,true),
  ('68000000-0000-0000-0000-000000000002','Other Tenant','other-tenant@test.local','92999990029','other-tenant','{"monday":{"active":true,"start":"09:00","end":"17:00"}}'::jsonb,'America/Manaus',30,0,true);

insert into public.services(id,tenant_id,name,price,duration_minutes,category,is_active,display_order)
values
  ('68000000-0000-0000-0000-000000000011','68000000-0000-0000-0000-000000000001','Servico de confirmacao',40,60,'Cabelo',true,1),
  ('68000000-0000-0000-0000-000000000012','68000000-0000-0000-0000-000000000002','Servico de confirmacao',40,60,'Cabelo',true,1);
...
select is((select customer_name from public.confirm_public_booking('confirmation-test','68000000-0000-0000-0000-000000000011','68000000-0000-0000-0000-000000000021','2040-01-02','09:00','Maria Silva','92999990027',null)),'Maria Silva'::text,'confirmation returns the submitted identity');
...
select is((select customer_name from public.confirm_public_booking('confirmation-test','68000000-0000-0000-0000-000000000011','68000000-0000-0000-0000-000000000021','2040-01-02','11:00','Nome Alterado','92999990027',null)),'Maria Silva'::text,'same phone reuses the existing canonical customer');
...
select is((select customer_name from public.confirm_public_booking('other-tenant','68000000-0000-0000-0000-000000000012','68000000-0000-0000-0000-000000000022','2040-01-02','09:00','Outra Pessoa','92999990027',null)),'Outra Pessoa'::text,'the same phone can identify a separate tenant customer');
...
create temp table confirmation_error(passed boolean);
do $$
begin
  perform * from public.confirm_public_booking('confirmation-test','68000000-0000-0000-0000-000000000011','68000000-0000-0000-0000-000000000021','2040-01-02','09:00','Pessoa Falha','92999990032',null);
  insert into confirmation_error values(false);
exception when sqlstate '23P01' then
  insert into confirmation_error values(true);
end $$;
