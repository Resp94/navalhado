-- Migration 062: preserva agendas legadas sem chave active explícita
...
BEGIN
  ...
  IF p_professional_id IS NOT NULL THEN
    SELECT
      ...
      COALESCE((prof.weekly_schedule -> v_day_of_week ->> 'active')::BOOLEAN, (prof.weekly_schedule -> v_day_of_week ->> 'start') IS NOT NULL AND (prof.weekly_schedule -> v_day_of_week ->> 'end') IS NOT NULL)
  ...
  WITH qualified_professionals AS (
    ...
    WHERE prof.tenant_id = p_tenant_id
      AND prof.is_active = true
      AND prof.deleted_at IS NULL
      AND COALESCE((prof.weekly_schedule -> v_day_of_week ->> 'active')::BOOLEAN, (prof.weekly_schedule -> v_day_of_week ->> 'start') IS NOT NULL AND (prof.weekly_schedule -> v_day_of_week ->> 'end') IS NOT NULL) IS TRUE
