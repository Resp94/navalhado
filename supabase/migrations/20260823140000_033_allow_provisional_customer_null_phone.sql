-- =============================================================================
-- Migration 033: Permitir telefone nulo temporário para clientes provisórios do link público
-- =============================================================================

-- 1. Permitir que clientes provisórios sejam criados com telefone nulo no primeiro acesso ao link público
ALTER TABLE public.customers ALTER COLUMN phone DROP NOT NULL;

-- 2. Ajustar check constraint de telefone normalizado para permitir clientes provisórios (cadastro_completo = false)
ALTER TABLE public.customers DROP CONSTRAINT IF EXISTS customers_telefone_normalizado_valid_chk;
ALTER TABLE public.customers ADD CONSTRAINT customers_telefone_normalizado_valid_chk 
  CHECK (cadastro_completo = false OR (telefone_normalizado IS NOT NULL));

-- 3. Atualizar RPC complete_customer_registration para preencher telefone e telefone_normalizado na conclusão
...
BEGIN
  ...
  IF p_phone IS NOT NULL AND btrim(p_phone) <> '' THEN
    ...
    -- Validar formato final: 55 + DDD (2 dígitos) + 8 ou 9 dígitos
    IF NOT v_clean_phone ~ '^55[1-9][0-9]{9,10}$' THEN
  ...
  SET name = v_name,
      ...
      telefone_normalizado = COALESCE(v_clean_phone, c.telefone_normalizado),
  ...
  SET name = v_name,
      ...
      telefone_normalizado = COALESCE(v_clean_phone, telefone_normalizado),
      updated_at = timezone('utc'::text, now())
  WHERE id = v_row.id;
