-- Ticket 08 da spec 037 (Fluxo de Caixa Projetado): Baixas como saída
-- realizada.
-- Spec: specs/037-fluxo-de-caixa-projetado/spec.md, seção "Fontes do
-- realizado, lidas nos livros de origem".
--
-- Cadeia sequencial (mesma função que os tickets 01, 02, 03, 05 e 07
-- redefinem, e último desta cadeia): create or replace de
-- public.get_projected_cash_flow e private.get_projected_cash_flow_core,
-- mesma assinatura de ambas.
--
-- A Baixa (public.payable_settlements, spec 036) conta pelo `paid_amount`
-- que a coluna gerada já grava (principal + juros - desconto), sem
-- recompor a fórmula: há uma única definição desse número, a mesma que a
-- tela de Contas a Pagar exibe. Conta na data do pagamento (`payment_date`,
-- já uma data de negócio -- `settle_payable` recusa data futura, então
-- nunca precisa de filtro de "pendente" como as demais fontes). Baixa
-- estornada (`reversed_at` preenchido) não conta; conta pelo próprio estado
-- de estorno, qualquer que seja o estado da Conta a Pagar (join com
-- `payables` só para a categoria, nunca para filtrar por status). Baixa de
-- valor pago zero (abatimento do fornecedor) soma zero aqui, e o saldo
-- restante da conta -- logo a Saída Prevista -- já cai pelo principal
-- abatido porque `settle_payable` incrementa `payables.paid_amount` pelo
-- principal, não pelo pago líquido (nenhuma mudança necessária no ticket 07
-- por causa disso). Baixa pela gaveta também gera `cash_movements`, mas o
-- fluxo nunca lê essa tabela -- conta uma vez só, mesma prova já feita para
-- sangria/suprimento no ticket 02. Nenhum índice novo: a consulta usa
-- `idx_payable_settlements_tenant_payment_date_active`, já criado pela spec
-- 036.
...
begin
  ...
  -- Saida Prevista e Conta a Pagar Vencida (ticket 07): mesma linha de
  -- Conta a Pagar em aberto ou parcialmente paga, com uma "data atribuida"
  -- (assigned_date) que decide o bucket -- o proprio vencimento quando ele
  -- e hoje ou depois (prevista), sempre p_today quando o vencimento ja
  -- passou (vencida, que so encontra bucket quando existe um "current").
  -- "overdue" marca qual das duas e, pra nunca contar a mesma conta nos
  -- dois campos nem lista-la duas vezes no detalhamento.
  payables_relevant as (
    select
      p.id,
      p.description,
      (p.amount - p.paid_amount) as remaining_amount,
      p.due_date,
      (p.due_date < p_today) as overdue,
      case when p.due_date < p_today then p_today else p.due_date end as assigned_date
    from public.payables p
    where p.tenant_id = p_tenant_id
      and p.status in ('open', 'partially_paid')
      and (
        p.due_date < p_today
        or p.due_date between p_start_date and p_end_date
      )
  ),
  -- Baixa (ticket 08): conta pelo "paid_amount" ja gerado (principal + juros
  -- - desconto), na data do pagamento -- sempre passada ou hoje, nunca
  -- futura (settle_payable recusa data futura), entao nao precisa de
  -- "pending" como as demais fontes. So pelo proprio reversed_at, qualquer
  -- que seja o status da conta -- nunca filtra por status de payables. O
  -- join com payables e so para pegar a categoria de despesa.
  daily_settlements as (
    select
      ps.payment_date as local_date,
      ps.paid_amount,
      p.category_id
    from public.payable_settlements ps
    join public.payables p on p.tenant_id = ps.tenant_id and p.id = ps.payable_id
    where ps.tenant_id = p_tenant_id
      and ps.reversed_at is null
      and ps.payment_date between p_start_date and p_end_date
  ),
  settlements_agg as (
    select local_date, sum(paid_amount) as total
    from daily_settlements
    group by local_date
  ),
  daily_settlements_by_category as (
    select
      ds.local_date,
      ds.category_id,
      coalesce(fc.name, 'Categoria removida') as category_name,
      sum(ds.paid_amount) as total
    from daily_settlements ds
    left join public.financial_categories fc on fc.id = ds.category_id
    group by ds.local_date, ds.category_id, coalesce(fc.name, 'Categoria removida')
  ),
  ...
  bucket_payables as (
    select
      b.bucket_start,
      b.bucket_end,
      coalesce(sum(pr.remaining_amount) filter (where not pr.overdue), 0.00) as outflow_forecast,
      coalesce(sum(pr.remaining_amount) filter (where pr.overdue), 0.00) as outflow_overdue
    from buckets b
    left join payables_relevant pr on pr.assigned_date between b.bucket_start and b.bucket_end
    group by b.bucket_start, b.bucket_end
  ),
  bucket_payables_list as (
    select
      b.bucket_start,
      b.bucket_end,
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'remaining_amount', round(x.remaining_amount, 2),
            'due_date', x.due_date,
            'overdue', x.overdue
          )
          order by x.due_date, x.description
        ) filter (where x.id is not null),
        '[]'::jsonb
      ) as payables_forecast
    from buckets b
    left join lateral (
      select pr.id, pr.description, pr.remaining_amount, pr.due_date, pr.overdue
      from payables_relevant pr
      where pr.assigned_date between b.bucket_start and b.bucket_end
    ) x on true
    group by b.bucket_start, b.bucket_end
  ),
  bucket_settlements as (
    select
      b.bucket_start,
      b.bucket_end,
      coalesce(sum(sa.total), 0.00) as settlements_total
    from buckets b
    left join settlements_agg sa on sa.local_date between b.bucket_start and b.bucket_end
    group by b.bucket_start, b.bucket_end
  ),
  bucket_settlements_by_category_distinct as (
    select
      b.bucket_start,
      b.bucket_end,
      coalesce(
        ...
        ) filter (where x.category_id is not null),
        '[]'::jsonb
      ) as settlements_by_category
    from buckets b
    left join lateral (
      select dsc.category_id, dsc.category_name, sum(dsc.total) as total
      from daily_settlements_by_category dsc
      where dsc.local_date between b.bucket_start and b.bucket_end
      group by dsc.category_id, dsc.category_name
    ) x on true
    group by b.bucket_start, b.bucket_end
  ),
  bucket_agg as (
    select
      ...
      bp.payouts_total + ba.advances_total + bs.settlements_total as outflow_realized,
      bpa.outflow_forecast,
      bpa.outflow_overdue,
      -- pending_flow soma o realizado com data posterior a hoje (entradas
      -- futuras somam, saidas futuras subtraem), a entrada estimada (so
      -- quando o historico e suficiente) e subtrai integralmente a Saida
      -- Prevista e a Vencida: nenhuma das duas ja saiu da gaveta, entao as
      -- duas sao sempre pendentes, mesmo a prevista vencendo hoje. A Baixa
      -- nunca e futura (settle_payable recusa data futura), entao nunca
      -- entra aqui -- so em outflow_realized. Compromissos sem Data NUNCA
      -- entram aqui.
      bi.inflow_pending - bp.payouts_pending - ba.advances_pending
        + (case when v_status = 'ok' then be.inflow_estimated_raw else 0 end)
        - bpa.outflow_forecast - bpa.outflow_overdue as pending_flow,
      bi.dinheiro,
      bi.pix,
      bi.cartao,
      bi.outros,
    ...
    join bucket_payables bpa on bpa.bucket_start = bi.bucket_start and bpa.bucket_end = bi.bucket_end
    join bucket_settlements bs on bs.bucket_start = bi.bucket_start and bs.bucket_end = bi.bucket_end
  ),
    ...
    'buckets', coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'outflow_forecast', round(ba.outflow_forecast, 2),
          'outflow_overdue', round(ba.outflow_overdue, 2),
          ...
          'detail', jsonb_build_object(
            ...
            'payables_forecast', bpl.payables_forecast,
            'settlements_by_category', bsc.settlements_by_category,
  ...
  join bucket_payables_list bpl
    on bpl.bucket_start = ba.bucket_start and bpl.bucket_end = ba.bucket_end
  join bucket_settlements_by_category_distinct bsc
    on bsc.bucket_start = ba.bucket_start and bsc.bucket_end = ba.bucket_end;
...
comment on function private.get_projected_cash_flow_core(uuid, date, date, text, date, text) is
  'Nucleo do fluxo de caixa projetado (spec 037): recebe "hoje" injetado, valida periodo e granularidade e agrega, por agrupamento, entradas realizadas de pagamento de Comanda, saidas realizadas de Quitacao de Comissao, vale e Baixa de Conta a Pagar, entrada estimada por dia da semana, Saida Prevista e Conta a Pagar Vencida. Devolve tambem os Compromissos sem Data, independentes do periodo pedido. Sem checagem de acesso.';
...
comment on function public.get_projected_cash_flow(uuid, date, date, text) is
  'Fluxo de Caixa Projetado (spec 037): entradas realizadas de Comandas fechadas, saidas realizadas de Quitacao de Comissao, vale e Baixa de Conta a Pagar, entrada estimada por dia da semana, Saida Prevista e Conta a Pagar Vencida, e Compromissos sem Data, por agrupamento de dia, semana ou mes. Superficie do gerente do proprio tenant; proprietario acessa qualquer tenant.';
