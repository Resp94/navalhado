-- Ticket 07 da spec 037 (Fluxo de Caixa Projetado): Saídas previstas e
-- Contas a Pagar Vencidas.
-- Spec: specs/037-fluxo-de-caixa-projetado/spec.md, seções "Dependência e
-- posição na sequência" e "Saídas previstas".
--
-- Cadeia sequencial (mesma função que os tickets 01, 02, 03, 05 e 08
...
--
-- Toda Conta a Pagar em aberto ou parcialmente paga (public.payables, spec
-- 036) gera saída prevista pelo saldo restante (amount - paid_amount), no
-- agrupamento que contém o vencimento, quando o vencimento é hoje ou
-- depois. Vencimento anterior a hoje é Conta a Pagar Vencida: entra sempre
-- no agrupamento atual (o que contém "hoje"), num campo separado
-- (outflow_overdue), nunca junto de outflow_forecast -- nenhuma conta é
-- contada nos dois. Conta paga ou cancelada não entra em nenhum dos dois.
--
-- Truque de junção: tanto prevista quanto vencida usam a mesma "data
-- atribuída" (assigned_date) para juntar com o bucket certo -- prevista usa
-- o próprio vencimento, vencida usa sempre p_today (que, por definição,
-- está dentro do bucket "current" quando ele existe). Um período inteiramente
-- passado não tem bucket "current" (p_end_date < p_today), então vencidas
-- não encontram bucket nenhum para se juntar -- nem previstas, porque
-- vencimento >= p_today > p_end_date nunca cai dentro do período. As duas
-- regras saem de graça da mesma junção, sem "if" separado para período
-- todo passado.
...
begin
  ...
  bucket_agg as (
    select
      ...
      -- duas sao sempre pendentes, mesmo a prevista vencendo hoje. Os
      -- tickets seguintes somam aqui a Baixa como saida realizada.
      -- Compromissos sem Data NUNCA entram aqui.
...
comment on function private.get_projected_cash_flow_core(uuid, date, date, text, date, text) is
  'Nucleo do fluxo de caixa projetado (spec 037): recebe "hoje" injetado, valida periodo e granularidade e agrega, por agrupamento, entradas realizadas de pagamento de Comanda, saidas realizadas de Quitacao de Comissao e vale, entrada estimada por dia da semana, Saida Prevista (Conta a Pagar em aberto pelo saldo restante, no bucket do vencimento) e Conta a Pagar Vencida (vencimento passado, sempre no bucket atual). Devolve tambem os Compromissos sem Data, independentes do periodo pedido. Sem checagem de acesso.';
...
comment on function public.get_projected_cash_flow(uuid, date, date, text) is
  'Fluxo de Caixa Projetado (spec 037): entradas realizadas de Comandas fechadas, saidas realizadas de Quitacao de Comissao e vale, entrada estimada por dia da semana, Saida Prevista e Conta a Pagar Vencida, e Compromissos sem Data, por agrupamento de dia, semana ou mes. Superficie do gerente do proprio tenant; proprietario acessa qualquer tenant.';
