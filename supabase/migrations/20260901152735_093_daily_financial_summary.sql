-- Migration 093: Resumo financeiro diário protegido por papel e tenant
-- Spec: specs/027-faturamento-diario-caixa/spec.md
...
BEGIN
  ...
  -- Força a validação do fuso antes de executar as agregações.
  ...
  closed_comandas AS (
    SELECT
      (c.closed_at AT TIME ZONE p_time_zone)::date AS local_date,
    ...
    WHERE c.tenant_id = v_target_tenant_id
      ...
      AND c.closed_at >= (p_start_date::timestamp AT TIME ZONE p_time_zone)
      AND c.closed_at < ((p_end_date + 1)::timestamp AT TIME ZONE p_time_zone)
    GROUP BY (c.closed_at AT TIME ZONE p_time_zone)::date
  ),
  payments AS (
    SELECT
      (cp.paid_at AT TIME ZONE p_time_zone)::date AS local_date,
    ...
    WHERE cp.tenant_id = v_target_tenant_id
      AND cp.paid_at >= (p_start_date::timestamp AT TIME ZONE p_time_zone)
      AND cp.paid_at < ((p_end_date + 1)::timestamp AT TIME ZONE p_time_zone)
      AND (p_cash_session_id IS NULL OR cp.cash_session_id = p_cash_session_id)
    GROUP BY (cp.paid_at AT TIME ZONE p_time_zone)::date
  )
  SELECT COALESCE(
    json_agg(
      json_build_object(
