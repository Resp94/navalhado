-- Ticket 11 da spec 038 (Modulo de Relatorios): Origem dos clientes, ultimo
-- ticket da pagina "Clientes" (mesmo contrato get_customer_report do ticket
-- 10). Spec: specs/038-modulo-de-relatorios/spec.md, secao "9-10.
-- get_customer_report" (bloco `registrations`) e Further Notes "Dado ralo no
-- canal de aquisicao e na data de nascimento".
--
-- CREATE OR REPLACE na MESMA assinatura de private.get_customer_report_core
-- e public.get_customer_report (ticket 10). Acrescenta a chave
-- `registrations` ao jsonb, sem tocar em nenhuma CTE nem chave existente
-- (visitors, previous_visitors, buckets, single_visit_customers idênticos).
--
-- Novas CTEs (todas derivadas de registrations_base, reduzida a uma linha
-- por cliente antes de qualquer agregacao -- nenhum join entre elas):
--   registrations_base: clientes do tenant com created_at no periodo (fuso
--     do tenant, intervalo meio-aberto, created_at comparado direto contra
--     bounds ja convertidos -- sem envolver a coluna em funcao, para uso do
--     indice novo).
--   registration_origin_agg: total/with_visit por registration_origin,
--     "with_visit" por EXISTS contra a CTE `visits` ja materializada no
--     nucleo (ticket 10) -- nao chama private.report_customer_visits de
--     novo.
--   channel_variants/channel_variant_counts/channel_display: acquisition_channel
--     e texto livre. Normaliza (trim+lower) para agrupar grafias diferentes
--     da mesma resposta; escolhe a grafia MAIS FREQUENTE para exibicao via
--     DISTINCT ON (norm_channel) ordenado por contagem desc (empate: ordem
--     alfabetica da grafia, deterministico). Nulo/vazio vira a chave
--     '__nao_informado__' internamente e o rotulo "Não informado" na saida.
--   acquisition_channel_agg: total/with_visit por grafia de exibicao.
--   acquisition_channel_final: acrescenta "Não informado" com total=0 quando
--     ele nao aparece em acquisition_channel_agg MAS ha cadastro no periodo
--     (destaque de qualidade do dado pedido pela spec: "sempre mostrado",
--     mesmo quando 100% dos canais estao preenchidos). Decisao sem
--     precedente explicito: com ZERO cadastro no periodo a lista fica
--     vazia (nada a destacar, mesmo padrao dos demais relatorios da spec,
--     que so listam o que existe) -- "Não informado" so aparece garantido
--     quando ha ao menos um cadastro no periodo.
--   registrations_totals: total/provisional (cadastro_completo = false) e
--     acquisition_channel_filled_share (fracao com canal preenchido sobre o
--     total do periodo; denominador zero devolve null via case, nunca zero
--     -- mesmo padrao de attendance_rate/cancellation_rate do ticket 07).
--
-- Indice: customers (tenant_id, created_at) nao existia (confirmado contra
-- pg_indexes no banco dev antes desta migracao) -- criado aqui, se ausente,
-- sem concurrently.
...
begin
  ...
  -- Ticket 11: cadastros do periodo (fuso do tenant, intervalo meio-aberto,
  -- created_at comparado direto contra os bounds -- sem funcao em cima da
  -- coluna, para usar idx_customers_tenant_created_at).
  registrations_base as (
    ...
    where c.tenant_id = p_tenant_id
      and c.created_at >= (p_start_date::timestamp at time zone p_timezone)
      and c.created_at < ((p_end_date + 1)::timestamp at time zone p_timezone)
  ),
  -- Origem do cadastro: automatica, sempre preenchida (registration_origin e
  -- not null na tabela) -- uma linha por origem presente no periodo.
  -- "with_visit" por EXISTS contra "visits" (ja materializada acima),
  -- nunca por join que multiplicaria a contagem se o cliente tiver varias
  -- Visitas.
  registration_origin_agg as (
    select
      rb.registration_origin as origin,
      count(*) as total,
      count(*) filter (
        where exists (select 1 from visits v where v.customer_id = rb.customer_id)
      ) as with_visit
    from registrations_base rb
    group by rb.registration_origin
  ),
  -- Canal de aquisicao: texto livre. Normaliza (trim+lower) para agrupar
  -- grafias diferentes da mesma resposta ("Instagram" e "instagram " juntas);
  -- nulo/vazio vira a chave interna '__nao_informado__'. A grafia de EXIBICAO
  -- e a MAIS FREQUENTE do grupo (nunca a normalizada), escolhida via
  -- DISTINCT ON ordenado por contagem desc (empate: ordem alfabetica da
  -- grafia, deterministico).
  channel_variants as (
    select
      rb.customer_id,
      coalesce(nullif(btrim(lower(rb.acquisition_channel)), ''), '__nao_informado__') as norm_channel,
      coalesce(nullif(btrim(rb.acquisition_channel), ''), 'Não informado') as raw_channel
    from registrations_base rb
  ),
  channel_variant_counts as (
    select norm_channel, raw_channel, count(*) as variant_count
    from channel_variants
    group by norm_channel, raw_channel
  ),
  channel_display as (
    select distinct on (norm_channel)
      norm_channel,
      raw_channel as display_channel
    from channel_variant_counts
    order by norm_channel, variant_count desc, raw_channel asc
  ),
  acquisition_channel_agg as (
    select
      cd.display_channel as channel,
      count(*) as total,
      count(*) filter (
        where exists (select 1 from visits v where v.customer_id = cv.customer_id)
      ) as with_visit
    from channel_variants cv
    join channel_display cd on cd.norm_channel = cv.norm_channel
    group by cd.display_channel
  ),
  -- "Não informado" sempre aparece quando ha cadastro no periodo (destaque de
  -- qualidade do dado pedido pela spec), mesmo quando 100% dos canais estao
  -- preenchidos (nenhuma linha "Não informado" em acquisition_channel_agg).
  -- Decisao sem precedente explicito na spec: com ZERO cadastro no periodo a
  -- lista fica vazia (nada a destacar, mesmo padrao dos demais relatorios da
  -- spec, que so listam o que existe) -- "Não informado" com total=0 so
  -- aparece quando ha ao menos um cadastro no periodo.
  acquisition_channel_final as (
    select channel, total, with_visit from acquisition_channel_agg
    union all
    select 'Não informado', 0, 0
    where (select count(*) from registrations_base) > 0
      and not exists (select 1 from acquisition_channel_agg where channel = 'Não informado')
  ),
  -- Totais do bloco: cadastros, provisorios (cadastro_completo = false) e
  -- fracao com canal preenchido. Denominador zero devolve null via case,
  -- nunca zero (mesmo padrao de attendance_rate/cancellation_rate, ticket
  -- 07).
  registrations_totals as (
    select
      count(*) as total,
      count(*) filter (where rb.cadastro_completo = false) as provisional,
      case
        when count(*) > 0
          then round((count(*) filter (where nullif(btrim(rb.acquisition_channel), '') is not null))::numeric / count(*), 4)
        else null
      end as acquisition_channel_filled_share
    from registrations_base rb
  )
  select jsonb_build_object(
    ...
    'registrations', jsonb_build_object(
      ...
      'by_registration_origin', coalesce(
        (select jsonb_agg(
          jsonb_build_object('origin', roa.origin, 'total', roa.total, 'with_visit', roa.with_visit)
          order by roa.origin
        ) from registration_origin_agg roa),
        '[]'::jsonb
      ),
      'by_acquisition_channel', coalesce(
        (select jsonb_agg(
          jsonb_build_object('channel', acf.channel, 'total', acf.total, 'with_visit', acf.with_visit)
          order by acf.total desc, acf.channel asc
        ) from acquisition_channel_final acf),
        '[]'::jsonb
      ),
      'acquisition_channel_filled_share', rt.acquisition_channel_filled_share
    )
  )
  into v_result
  from visitors_agg va, unidentified_agg ua, prev_visitors_agg pva, prev_unidentified_agg pua, registrations_totals rt;
...
comment on function private.get_customer_report_core(uuid, date, date, text, date, text) is
  'Nucleo de Novos x recorrentes + Origem dos clientes (spec 038, tickets 10 e 11): usa private.report_customer_visits UMA VEZ. Cliente Novo = primeira Visita da vida cai no periodo; Recorrente = teve Visita no periodo e ja tinha Visita antes do inicio. Buckets: Novo pela primeira Visita da vida, Recorrente pela primeira Visita dele dentro do periodo. Cliente de Uma Visita = Novo do periodo com so uma Visita ate p_today (lista ate 200, mais recentes primeiro). Atendimentos sem cliente identificado = Comandas fechada sem customer_id, contadas direto (Visita ja exige customer_id). previous_visitors nao tem new_single_visit (depende de "ate hoje", incompativel com periodo anterior fixo). registrations: cadastros do periodo por origem (automatica) e canal de aquisicao (declarado, texto normalizado trim+lower, exibido pela grafia mais frequente, nulo/vazio "Não informado" sempre presente quando ha cadastro no periodo); with_visit por EXISTS contra visits; acquisition_channel_filled_share null quando nao ha cadastro no periodo. So p_today injetado (sem p_now: nada aqui classifica por instante). Sem checagem de acesso.';
...
-- em qualquer tenant), mensagens de erro identicas ao ticket 01. Sem
-- mudanca de assinatura nem de corpo alem do repasse (o nucleo e quem ganhou
-- o campo novo) -- CREATE OR REPLACE so para manter a comment atualizada.
...
comment on function public.get_customer_report(uuid, date, date, text) is
  'Novos x recorrentes + Origem dos clientes (spec 038, tickets 10 e 11): visitantes unicos/novos/recorrentes/novos-de-uma-visita/sem-cliente do periodo e do anterior, agrupamento por dia/semana/mes, lista de Clientes de Uma Visita (ate 200) e cadastros do periodo por origem/canal de aquisicao. Reusa a Visita do ticket 09. Gerente do proprio tenant (tenant nulo recusado); proprietario acessa qualquer tenant.';
