-- Migration 070: prioriza a configuração de horário em português.
--
-- A agenda interna grava os horários do estabelecimento nas chaves em
-- português (ex.: sexta). Alguns tenants legados também possuem a chave
-- inglesa equivalente (ex.: friday), com valores diferentes. O canal público
-- e o reagendamento devem usar a mesma configuração da agenda interna.
...
BEGIN
  ...
  SELECT
    COALESCE(t.business_hours -> v_day_bh_key ->> 'open', t.business_hours -> v_day_bh_key ->> 'start', t.business_hours -> v_day_of_week ->> 'start', '08:00'),
    COALESCE(t.business_hours -> v_day_bh_key ->> 'close', t.business_hours -> v_day_bh_key ->> 'end', t.business_hours -> v_day_of_week ->> 'end', '20:00'),
    COALESCE((t.business_hours -> v_day_bh_key ->> 'active')::BOOLEAN, (t.business_hours -> v_day_of_week ->> 'active')::BOOLEAN, true)
