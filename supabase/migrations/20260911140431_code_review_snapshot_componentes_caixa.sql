-- Persiste a fotografia completa do fechamento do caixa e a versão da regra.
alter table public.cash_sessions
  add column if not exists cash_received_amount numeric(10,2) check (cash_received_amount is null or cash_received_amount >= 0),
  add column if not exists pix_received_amount numeric(10,2) check (pix_received_amount is null or pix_received_amount >= 0),
  add column if not exists card_received_amount numeric(10,2) check (card_received_amount is null or card_received_amount >= 0),
  add column if not exists other_received_amount numeric(10,2) check (other_received_amount is null or other_received_amount >= 0),
  add column if not exists payment_count integer check (payment_count is null or payment_count >= 0),
  add column if not exists supplies_amount numeric(10,2) check (supplies_amount is null or supplies_amount >= 0),
  add column if not exists withdrawals_amount numeric(10,2) check (withdrawals_amount is null or withdrawals_amount >= 0),
  add column if not exists calculation_version text;

update public.cash_sessions
set calculation_version = 'cash_expected_v1'
where calculation_version is null;

alter table public.cash_sessions
  alter column calculation_version set default 'cash_expected_v1';
...
begin
  ...
  set closed_by = v_user_id,
      ...
      calculation_version = 'cash_expected_v1',
      status = 'closed',
