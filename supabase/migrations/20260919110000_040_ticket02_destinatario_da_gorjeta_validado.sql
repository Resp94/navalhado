-- Spec 040, ticket 02: o destinatario da gorjeta precisa ter atendido na Comanda.
--
-- settle_comanda recusa destinatario que nao seja profissional da unidade e
-- destinatario que nao aparece em nenhum Item de Comanda. Gorjeta maior que
-- zero com dois ou mais profissionais nos itens exige destinatario. O gatilho
-- de credito na Conta do Profissional tambem confere a unidade, para que
-- nenhum caminho credite gorjeta a profissional de outra barbearia.
--
-- Mudancas sobre a versao do ticket 01: o bloco de validacao do destinatario
-- logo apos a resolucao de preco, e a guarda no gatilho (fim do arquivo).
...
begin
  if new.status = 'fechada' and old.status is distinct from new.status
    ...
    perform 1
    from public.professionals
    where id = new.tip_professional_id
      and tenant_id = new.tenant_id;

    if not found then
      raise exception 'O destinatário da gorjeta não pertence à unidade da comanda.' using errcode = 'P0001';
    end if;
