-- Ticket 03 da spec 034 (expand): prepara o tipo de movimentacao de vale de
-- profissional nas formulas de gaveta ANTES de qualquer feature gera-lo (ticket 05).
--
-- Todas as formulas de saldo disponivel e de valor esperado da gaveta filtram
-- cash_movements por tipo de forma nominal e explicita. Um tipo novo que nao seja
-- acrescentado a cada uma delas sairia da gaveta sem ser subtraido, superestimando
-- o saldo disponivel e o valor esperado no fechamento -- reproduzindo exatamente o
-- defeito que a migration 20260912060000 corrigiu para repasse_comissao (expected_amount
-- negativo tornando o fechamento impossivel de concluir).

alter table public.cash_movements
  drop constraint if exists cash_movements_type_check;
alter table public.cash_movements
  add constraint cash_movements_type_check
  check (type = any (array['sangria', 'suprimento', 'repasse_comissao', 'vale_profissional']));

-- close_cash_session: valor esperado no fechamento passa a descontar vale de
-- profissional pago em dinheiro, da mesma forma que ja desconta repasse de comissao.
-- Versao de calculo incrementada para 'cash_expected_v3', marcando de forma auditavel
-- que sessoes fechadas a partir daqui ja consideram o vale na conferencia.
...
-- register_commission_payout: saldo disponivel na gaveta para quitacao em dinheiro
-- passa a descontar tambem os vales de profissional pagos em dinheiro no turno,
-- pelo mesmo motivo -- sem isso, um vale em dinheiro sairia da gaveta sem ser
-- subtraido, superestimando o saldo disponivel para a quitacao seguinte.
