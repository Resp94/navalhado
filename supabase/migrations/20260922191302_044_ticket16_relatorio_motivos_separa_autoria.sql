-- Ticket 16 da spec 044: o ranking de motivos de cancelamento do relatorio de
-- Agenda (private.get_schedule_report_core / public.get_schedule_report,
-- spec 038 tickets 07/08) passa a separar por autoria do cancelamento
-- (public.appointments.canceled_by, spec 043 ticket 06): 'shop', 'customer'
-- e 'desconhecida' (cancelamento anterior a spec 043, canceled_by nulo --
-- NUNCA inferido pelo texto do motivo).
--
-- Chave 'cancellation_reasons' muda de array plano para objeto com as tres
-- chaves fixas shop/customer/desconhecida, cada uma com o mesmo formato de
-- antes (top 10 por contagem desc/motivo asc + 'outros' para o resto),
-- agora particionado por grupo em vez de global.
--
-- Texto de preenchimento do Canal do Cliente ("Cancelado pelo cliente",
-- gravado quando o cliente cancela sem escrever motivo -- definicao unica em
-- MOTIVO_CANCELAMENTO_PADRAO_CLIENTE, src/modules/canal-cliente/types.ts)
-- deixa de contar como motivo no ranking: nao diz nada sobre o motivo real,
-- so tende a ocupar o topo por ser o preenchimento mais comum. Comparacao
-- normalizada (trim+lower), a mesma ja aplicada ao proprio motivo. Esses
-- cancelamentos continuam contados em status_totals.canceled (CTE separada,
-- nao tocada aqui) -- so somem do ranking de motivos, nao do total.
--
-- Motivo em branco continua "sem motivo informado", agrupado dentro do grupo
-- de autoria do cancelamento (comportamento inalterado, so agora por grupo).
...
comment on function private.get_schedule_report_core(uuid, date, date, uuid, date, timestamptz, text) is
  'Nucleo do relatorio de Agenda: comparecimento, cancelamento, no-show, mapa de calor e motivos de cancelamento por autoria (spec 038, tickets 07 e 08; spec 044, ticket 16). Recebe "hoje" (dia de negocio) e "agora" (instante) injetados. Classifica cada Agendamento com start_time no periodo em completed/no_show/canceled/unresolved (sem desfecho: pending/confirmed/in_progress com start_time < p_now)/future (mesmos status com start_time >= p_now); canceled nunca vira future/unresolved. Taxas com denominador zero devolvem null. p_professional_id filtra status_totals/by_origin/cancellation_reasons/heatmap, mas NAO filtra by_professional (inverso da regra do ticket 05/06). cancellation_reasons e um objeto {shop, customer, desconhecida} (autoria de public.appointments.canceled_by; nulo = desconhecida, cancelamento anterior a spec 043, nunca inferido pelo texto), cada grupo normalizado (trim+lower), vazio como "sem motivo informado", top 10 + "outros", e excluindo o texto de preenchimento do Canal do Cliente ("cancelado pelo cliente" normalizado) do ranking -- esses cancelamentos continuam em status_totals.canceled. heatmap: cells{weekday,hour,count} conta Agendamentos NAO cancelados (faltas contam) por dia da semana (extract(dow): 0=domingo..6=sabado) e hora de start_time no fuso do tenant, so combinacoes com pelo menos 1 Agendamento (ausente = 0); hours e o intervalo [menor abertura, maior fechamento] entre os dias ativos de tenants.business_hours, ampliado (nunca reduzido) por Agendamento fora do expediente ou com expediente ausente/inativo. Sem checagem de acesso.';
...
comment on function public.get_schedule_report(uuid, date, date, uuid) is
  'Agenda (spec 038, tickets 07 e 08; spec 044, ticket 16): totais por status do periodo e do anterior (concluidos, faltas, cancelados, sem desfecho, futuros, taxa de comparecimento, taxa de cancelamento), por origem, por profissional (sempre completa, inclusive inativos/arquivados), motivos de cancelamento separados por autoria (shop/customer/desconhecida) e mapa de calor por dia da semana e hora (heatmap.hours e heatmap.cells{weekday,hour,count}, weekday 0=domingo..6=sabado, so nao cancelados, fuso do tenant). Gerente do proprio tenant (tenant nulo recusado); proprietario acessa qualquer tenant. p_professional_id filtra tudo, exceto by_professional.';
