-- Spec 040, ticket 03: desconto percentual registrado na Comanda.
--
-- A Comanda passa a guardar o tipo (amount | percent) e o percentual original
-- do desconto, alem do valor em reais. settle_comanda e settle_comanda_idempotent
-- ganham p_discount_percent no fim: quando informado (0 a 100), o BANCO converte
-- o percentual em reais a centavo sobre o subtotal e ignora p_discount_amount,
-- para o total gravado ser sempre o que a tela calcula com a mesma regra.
--
-- A assinatura muda, entao as antigas sao removidas: duas assinaturas coexistiriam
-- e o PostgREST (que resolve por nome) ficaria sujeito a ambiguidade, o mesmo
-- raciocinio da migration 20260912080000.
...
alter table public.comandas
  drop constraint if exists comandas_discount_type_check,
  add constraint comandas_discount_type_check check (discount_type in ('amount', 'percent')),
  drop constraint if exists comandas_discount_percent_check,
  add constraint comandas_discount_percent_check check (
    (discount_type = 'percent' and discount_percent is not null and discount_percent between 0 and 100)
    or (discount_type = 'amount' and discount_percent is null)
  );

drop function if exists public.settle_comanda(uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid);
drop function if exists public.settle_comanda_idempotent(uuid, uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid);
...
begin
  ...
  -- Preco efetivo: o catalogo manda. Item cujo produto ou servico nao e
  -- encontrado (ou esta inativo) segue com o preco enviado e e recusado adiante.
  select
    jsonb_agg(
      case
  ...
  if p_tip_professional_id is not null then
    perform 1
    from public.professionals
    where id = p_tip_professional_id
      and tenant_id = p_tenant_id;
    ...
    if not exists (
      ...
      where nullif(entry.payload->>'professional_id', '')::uuid = p_tip_professional_id
    ) then
      raise exception 'O destinatário da gorjeta deve ser um profissional que atendeu na comanda.' using errcode = 'P0001';
    end if;
  elsif coalesce(p_tip_amount, 0) > 0
    and (
      select count(distinct nullif(entry.payload->>'professional_id', ''))
      from jsonb_array_elements(p_itens) as entry(payload)
    ) >= 2
  then
...
revoke all on function public.settle_comanda(uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid, numeric) from public, anon;
grant execute on function public.settle_comanda(uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid, numeric) to authenticated;
...
revoke all on function public.settle_comanda_idempotent(uuid, uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid, numeric) from public, anon;
grant execute on function public.settle_comanda_idempotent(uuid, uuid, uuid, uuid, uuid, numeric, numeric, uuid, jsonb, jsonb, uuid, numeric) to authenticated;
