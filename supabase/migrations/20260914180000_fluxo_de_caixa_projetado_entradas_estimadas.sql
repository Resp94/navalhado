-- Ticket 03 da spec 037 (Fluxo de Caixa Projetado): Entradas estimadas por
-- dia da semana.
-- Spec: specs/037-fluxo-de-caixa-projetado/spec.md, secao "Entradas
-- estimadas: media por dia da semana".
--
-- Cadeia sequencial (mesma funcao que os tickets 01, 02, 05, 07 e 08
...
comment on function private.get_projected_cash_flow_core(uuid, date, date, text, date, text) is
  'Nucleo do fluxo de caixa projetado (spec 037): recebe "hoje" injetado, valida periodo e granularidade e agrega, por agrupamento, entradas realizadas de pagamento de Comanda, saidas realizadas de Quitacao de Comissao e vale, e entrada estimada por dia da semana (media das ultimas N semanas, N ate 8). Sem checagem de acesso.';
...
comment on function public.get_projected_cash_flow(uuid, date, date, text) is
  'Fluxo de Caixa Projetado (spec 037): entradas realizadas de Comandas fechadas, saidas realizadas de Quitacao de Comissao e vale, e entrada estimada por dia da semana, por agrupamento de dia, semana ou mes. Superficie do gerente do proprio tenant; proprietario acessa qualquer tenant.';
