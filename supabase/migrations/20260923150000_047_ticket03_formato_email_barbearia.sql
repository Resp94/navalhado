-- Spec 047, ticket 03: formato rigido no e-mail da barbearia (cadastro e
-- Configuracoes).
--
-- CHECK em tenants.email com a funcao unica public.email_valido (ticket 01).
-- O trigger handle_new_user (migration 089) passa a usar a mesma funcao no
-- lugar da regex copiada, mantendo o codigo de erro INVALID_TENANT_EMAIL.

alter table public.tenants
  add constraint tenants_email_format_check
  check (public.email_valido(email))
  not valid;

alter table public.tenants
  validate constraint tenants_email_format_check;
