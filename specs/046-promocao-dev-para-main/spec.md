...
- **Provas no navegador, em produção (tenant Barber Tester), 2026-09-23:**
  ...
  6. ⛔ **Não verificado.** Minha Agenda do barbeiro (grade travada, Bloquear só ele): bloqueado por bug pré-existente e não relacionado à promoção — o botão "Criar acesso" da tela Equipe chama a Edge Function `create-barber-access`, que não existe nem em produção nem em dev (só `whatsapp-integration` e `public-customer-session` estão deployadas). Profissional de teste "Prova Promocao Barbeiro" criado e removido de volta (histórico preservado, sem login gerado). Bug sinalizado como tarefa separada (spawn_task `task_fbfb3f65`), fora do escopo desta promoção.
  ...
  - Detalhe: issue `06`.
- **Pendências fora do escopo desta spec, registradas como tarefas separadas:** correção da Edge Function `create-barber-access` ausente (item 6 acima).
